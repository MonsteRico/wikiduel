...
- Ordinary article: request `Ada Lovelace`; verify the canonical title, prose,
  attribution, node counts, and a successful cache hit when it is requested a
  second time.
...
- Reported-strange article: request the title from the current content report;
  record the requested/canonical identity, revision, omission reasons, cache
  outcome, and timing from Diagnostics, then reduce any new shape to a small
  fixture before changing the normalizer.
