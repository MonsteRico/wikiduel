...
Use this prompt in a fresh chat for a ticket that has already been planned and marked `ready-for-agent`.

```text
Begin /implement for:

Ticket: [TICKET LINK]
Parent spec: [SPEC LINK, or "none"]
Base branch: [main or dependency branch]
Active parallel work: [none, or list each active ticket, branch, and ownership area]

This ticket has already been planned and marked ready-for-agent. Do not run
...
or add dependencies unless required by the ticket, and do not modify global
development tooling.

Before editing, perform a brief readiness audit:

1. Read the complete ticket, parent spec, CONTEXT.md, relevant ADRs, dependency
   tickets, and the current implementation.
...
5. Check that the ticket remains consistent with the current codebase.
...
reopen decisions already resolved by the ticket and spec.
...
- Keep the change within the ticket's acceptance criteria and explicit scope.
- Preserve unrelated user changes and avoid opportunistic refactors.
- If adjacent work is useful but not required, propose a follow-up ticket
...
2. Run /implement's Standards and Spec review against the final diff, ticket,
   spec, glossary, and ADRs.
...
- Change the local Markdown ticket to `Status: completed` within this branch.
...
- Commit the ticket-state change as part of the implementation PR so code,
...
- Open a PR containing:
  ...
  - known risks and proposed follow-up tickets.
- If the pull-request URL was not known when the ticket was first marked
  completed, update its `Completed:` line with that link and push the tracker
  update to the same PR branch.

Only mark the ticket completed after implementation, review, and all required
...
it is expected for the ticket to read completed on the PR branch while awaiting
